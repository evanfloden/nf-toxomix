# NF-toxomix Installation

To start using the NF-toxomix pipeline, there are three steps described below:
...
This pipeline itself needs no installation - NextFlow will automatically fetch it from GitHub if `skptic/NF-toxomix` is specified as the pipeline name.
...
```bash
wget https://github.com/skptic/NF-toxomix/archive/master.zip
unzip master.zip -d /my-pipelines/
cd /my_data/
nextflow run /my-pipelines/nf-toxomix-master
```
