...
data_dir=${data_path}/nf-toxomix_test_set
...
then
    ...
    curl https://your_website.com/nf-toxomix_test_set.tar.bz2 > ${data_path}/nf-toxomix_test_set.tar.bz2
    echo "Unpacking test set..."
    tar xvjf ${data_path}/nf-toxomix_test_set.tar.bz2 -C ${data_path}
    echo "Done"
fi

# Run name
run_name="Test NF-toxomix Run: "$(date +%s)
